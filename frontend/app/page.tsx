...
import { Search, Smartphone, Laptop, Headphones, ArrowRight, Star, Mic, User, LogOut } from 'lucide-react'
...
function HomepageDarkAnimated() {
  ...
  const handleVoiceSearch = () => {
    ...
    recognition.onresult = (event) => {
      try {
        ...
        if (transcript.trim()) {
          router.push(`/devices/${encodeURIComponent(transcript)}`)
        }
      } catch (err) {
    ...
    recognition.onerror = (event) => {
      ...
      setError('Voice recognition is not available or failed.')
    }
  }
  ...
  const handleSearch = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && searchProduct.trim()) {
      try {
        const sanitizedSearch = DOMPurify.sanitize(searchProduct.trim())
        router.push(`/devices/${encodeURIComponent(sanitizedSearch)}`)
      } catch (err) {
